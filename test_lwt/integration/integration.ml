...
let produce brokers topic_name keyed_messages =
  ...
  let send (key, msg) = Kafka_lwt.produce topic ~key msg in
  let loop () = Lwt_list.iter_s send keyed_messages in
  let term () =
...
let consume brokers topic_name messages =
  ...
  in
  let start () = partitions |> List.iter start_partition |> return in
  let rec loop () =
    if Cache.is_empty !expected_messages then return ()
    else Kafka_lwt.consume_queue queue >>= remove_received >>= loop
  in
  let term () =
...
let timeout s err =
  ...
  Lwt_io.printf "%s\n%!" err >>= fun () -> Lwt.fail (Failure "timeout")
...
let test_produce_consume () =
  let messages =
    [
      random_msg (); random_msg (); random_msg (); random_msg (); random_msg ();
    ]
  in
  Lwt.join
    [
...
let () =
  Lwt_main.run
    (List.fold_left Lwt.bind (return ())
       [
         (fun () -> Lwt_io.printf "%s\n%!" "Start LWT tests");
